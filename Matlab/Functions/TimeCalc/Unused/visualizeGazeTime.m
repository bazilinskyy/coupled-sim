...
visAllGroupCountsV2(data.groupcounts)
end

%% Helper functions
function [titlestr,ylab,labelstr,colour,yielding,mapstr] = getTitleString(con, mapping, person,setting)
if(strcmp(con,'ND_Y'))
...
function [lab,mapstr] = visGroupCounts(data, con, mapping, person,setting)
...
% Titlestring
[titlestr,ylabelstr,lab,colour,yielding,mapstr] = getTitleString(con, mapping, person, setting);
% Rect data
...
for p=1:length(fld_phase)-minus
    ...
    if(length(xdata)>2 && setting == 1)
        a = plot(xdata(3:end),ydata(3:end),'LineWidth',2,'Color',colour); % start from 3 to avoid '-1/-8' and '0'
    elseif (length(xdata)>2 && setting == 2)
        a = plot(xdata(3:end),ydata(3:end),'LineWidth',1,'Color',colour); % start from 3 to avoid '-1/-8' and '0'
    end
...
for per=2 %1:length(fld_person)
    for c=1:length(idx)
        ...
        for m=1:length(fld_map)
            subplot(3,1,m);
            hold on;
            [a,~] = visGroupCounts(data, fld_con{idx(c,1)}, fld_map{m}, fld_person{per},1);
            [b,~] = visGroupCounts(data, fld_con{idx(c,2)}, fld_map{m}, fld_person{per},1);
...
function visAllGroupCountsV2(data)
fld_person = fieldnames(data.D_NY.map0);
fld_con = fieldnames(data);
...
for per=2 %1:length(fld_person)
    for c=1:length(fld_con)
        ...
        for m=1:length(fld_map)
            hold on;
            [~,a{m}] = visGroupCounts(data, fld_con{c}, fld_map{m}, fld_person{per},2);
